...
const BibleReader: React.FC<BibleReaderProps> = ({ className = '' }) => {
  ...
  const [activeTab, setActiveTab] = useState<'read' | 'search' | 'notes'>('read');
  ...
  useEffect(() => {
    if (selectedBook && selectedChapter) {
      const chapterVerses = getVersesByChapter(selectedBook, selectedChapter);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                id="book-select"
                value={selectedBook || ''}
                onChange={(e) => setSelectedBook(Number(e.target.value) || null)}
          ...
          {verses.length > 0 && (
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              <div className="space-y-3">
                {verses.map((verse) => {
                  const verseNotes = getVerseNotes(verse.book, verse.chapter, verse.verse);
                  return (
                    <div
                    ...
                    >
                      <div className="flex items-start space-x-3 p-3 rounded-md hover:bg-gray-50">
                        ...
                        <div className="flex-1">
                          <p className="text-gray-800 leading-relaxed verse-text">{verse.text}</p>
                          {verseNotes.length > 0 && (
                            <div className="mt-2 space-y-1">
                              {verseNotes.map((note) => (
                                <div key={note.id} className="bg-yellow-50 border-l-4 border-yellow-400 p-2 rounded">
                                  <p className="text-sm text-gray-700">{note.note}</p>
                                  <div className="flex justify-between items-center mt-1">
                                    <span className="text-xs text-gray-500">
                                      {note.updated.toLocaleDateString('th-TH')}
                                    </span>
                                    <button
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              <div className="space-y-4">
                {searchResults.map((verse) => (
                  <div key={`${verse.book}-${verse.chapter}-${verse.verse}`} className="border-b border-gray-200 pb-4 last:border-b-0">
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="font-medium text-blue-600">
                        {formatVerseReference(verse.book_name, verse.chapter, verse.verse)}
                      </h4>
                      <button
                        onClick={() => openNoteModal(verse)}
      ...
      {activeTab === 'notes' && (
        <div className="space-y-6">
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            {notes.length === 0 ? (
              <p className="text-gray-500 text-center py-8">ยังไม่มีโน้ต</p>
            ) : (
              <div className="space-y-4">
                {notes
                  ...
                  .map((note) => (
                    <div key={note.id} className="border rounded-lg p-4">
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="font-medium text-blue-600">
                          {formatVerseReference(note.bookName, note.chapter, note.verse)}
                        </h4>
                        <button
                          onClick={() => handleDeleteNote(note.id)}
                      ...
                      <p className="text-xs text-gray-500">
                        อัพเดทล่าสุด: {note.updated.toLocaleDateString('th-TH')} {note.updated.toLocaleTimeString('th-TH')}
                      </p>
                    </div>
                  ))}
              </div>
      ...
      {showNoteModal && selectedVerse && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">
              เพิ่มโน้ตสำหรับ {formatVerseReference(selectedVerse.book_name, selectedVerse.chapter, selectedVerse.verse)}
            </h3>
