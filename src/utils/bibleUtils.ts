...
import type { BibleVerse, BibleBook, BibleNote, ThaiKJVData } from '../types/bible';
...
export function getSpecificVerse(bookId: number, chapter: number, verse: number): BibleVerse | undefined {
  return typedBibleData.verses.find((v: BibleVerse) => 
...
export function searchVerses(query: string): BibleVerse[] {
  ...
  return typedBibleData.verses.filter((verse: BibleVerse) => 
...
export class BibleNotesManager {
  ...
  static saveNote(note: Omit<BibleNote, 'id' | 'created' | 'updated'>): BibleNote {
  ...
  static updateNote(id: string, updates: Partial<BibleNote>): BibleNote | null {
    const notes = this.getNotes();
  ...
  static getNotesByVerse(bookId: number, chapter: number, verse: number): BibleNote[] {
  ...
  static getNotesByChapter(bookId: number, chapter: number): BibleNote[] {
    return this.getNotes().filter(note => 
      note.bookId === bookId && note.chapter === chapter
    );
  }
}
...
export function getBookIdByName(bookName: string): number | undefined {
  const verse = typedBibleData.verses.find((v: BibleVerse) => v.book_name === bookName);
  return verse?.book;
}

// ฟังก์ชันสำหรับการแปลง book id เป็น book name
export function getBookNameById(bookId: number): string | undefined {
  const verse = typedBibleData.verses.find((v: BibleVerse) => v.book === bookId);
