...
test('search for createToolTesting', async ({ page }) => {
  ...
  await page.getByPlaceholder('Search for anything...').fill('createToolTesting');

  // Verify the input value
  await expect(page.getByPlaceholder('Search for anything...')).toHaveValue('createToolTesting');

  // Wait for and verify search results
  await expect(page.getByText("Found 1 result for 'createToolTesting'")).toBeVisible();

  // Verify the search result title appears
  await expect(page.getByRole('link', { name: 'DiaryCoding:CreateToolTesting' })).toBeVisible();
...
test('search for behindMyCode', async ({ page }) => {
  ...
  // Verify the input value
  await expect(page.getByPlaceholder('Search for anything...')).toHaveValue('behindMyCode');

  // Wait for and verify search results
  await expect(page.getByText("Found 1 result for 'behindMyCode'")).toBeVisible();

  // Verify the search result title appears
  await expect(page.getByRole('link', { name: 'DiaryCoding:behindMyCode' })).toBeVisible();
  ...
  await expect(page).toHaveURL('https://codegeassmasterzeronebreakoutr1r2.codes/posts/behindMyCode1/');
